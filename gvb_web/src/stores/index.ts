...
export const useStore = defineStore('counter', {
    state() {
        return {
            ...
            userInfo: userInfo,
            siteInfo: siteInfo,
            bigModelInfo: TypeBigModel.options(),
        }
    },
    actions: {
        // 黑白主题持久化
        ...
        async loadToken() {
            ...
            if (value === null) {
                // 未登录
                Message.warning("未登录")
                let login = await ApiUser.LoginEmail({user_name: "youke", password: "youke",})
                await this.setToken(login.data)
                Message.success("自动登录游客账号")
                return;
            }
            try {
        ...
        async getBigModelInfo() {
            ...
            Object.assign(this.bigModelInfo, result.data)
        }
    },
    getters: {
        isLogin(): boolean {
